...
int getDrumIndexForCurrentScreen(mpc::Mpc &mpc, const std::shared_ptr<ScreenComponent> screen)
{
    ...
    const int result = isSamplerScreen ? mpc.screens->get<screens::DrumScreen>()->getDrum() : mpc.getSequencer()->getActiveTrack()->getBus() - 1;
    return result;
}
...
TriggerDrumNoteOffContext TriggerDrumContextFactory::buildTriggerDrumNoteOffContext(
    ...
    const std::shared_ptr<ScreenComponent> screen)
{
    std::function<void()> finishBasicVoiceIfSoundIsLooping = [basicPlayer = &mpc.getBasicPlayer()]()
    {
    ...
    std::function<bool()> isAnyProgramPadRegisteredAsPressed = [sampler = mpc.getSampler()]
    {
    ...
    std::function<int()> getActiveSequenceLastTick = [sequencer = mpc.getSequencer()]
    {
        return sequencer->getActiveSequence()->getLastTick();
    };

    std::function<void(double)> sequencerMoveToQuarterNotePosition = [sequencer = mpc.getSequencer()](double quarterNotePosition)
    {
        sequencer->move(quarterNotePosition);
    };

    std::function<void()> sequencerStopMetronomeTrack = [sequencer = mpc.getSequencer()]
    {
