...
Parameter::Parameter(mpc::Mpc &mpc, std::string labelStr, std::string name, int x, int y, int fieldWidth)
    : Component(name)
{
    const auto labelWidth = mpc::Util::getTextWidthInPixels(labelStr);

    addChild(std::make_shared<Label>(mpc, name, labelStr, x, y - 1, labelWidth));
    ...
    addChild(std::make_shared<Field>(mpc, name, x + labelWidth, y, fieldWidth + 1));
}
