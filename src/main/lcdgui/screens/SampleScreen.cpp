    ...
    : ScreenComponent(mpc, "sample", layerIndex)
{
    addReactiveBinding({
        ...
        {
            return {
                mpc.getAudioMidiServices()->isRecordingSound(),
        ...
        [&](auto isRecordingAndisArmed)
        {
            ...
            if (isRecordingAndisArmed.first) b->setBackgroundName("recording");
            else if (isRecordingAndisArmed.second) b->setBackgroundName("waiting-for-input-signal");
            else b->setBackgroundName("sample");
        }
    });
    addReactiveBinding({
        [&]()->std::pair<float, float>{ return { currentBufferPeakL, currentBufferPeakR };},
        [&](auto) { updateVU(); }
    });
}
...
void SampleScreen::setCurrentBufferPeak(const std::pair<float, float> &peakToUse)
{
