...
void VmpcMidiPresetsScreen::open()
{
    ...
    if (row + rowOffset >= (mpc.presets.size() + 1))
    {
        row = 0;
        rowOffset = 0;
    }
...
void VmpcMidiPresetsScreen::displayUpAndDown()
{
    ...
    findChild<Label>("down")->Hide(rowOffset + 4 >= mpc.presets.size());
}
...
{
    if (row + rowOffset >= mpc.presets.size()) return;
...
{
    if (column == 1 || (row + rowOffset == 0 && mpc.presets.empty())) return;
