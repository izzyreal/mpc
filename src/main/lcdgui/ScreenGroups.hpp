...
namespace mpc::lcdgui::screengroups
{
    ...
    bool isScreenThatIsNotAllowedToOpenMainScreen(const std::shared_ptr<ScreenComponent> &);
    bool isPlayOnlyScreen(const std::shared_ptr<ScreenComponent> &);
    bool isCentralNoteAndPadUpdateScreen(const std::shared_ptr<ScreenComponent> &);
