...
EventRow::EventRow(int bus, weak_ptr<mpc::sequencer::Event> e, int rowIndex)
	: Component("event-row"), sampler(Mpc::instance().getSampler()), rowIndex(rowIndex)
{
	if (bus != 0)
	{
...
void EventRow::initLabelsAndFields()
{
	...
	for (int i = 0; i < 5; i++)
	{
		auto candidate = layer->findField(letters[i] + to_string(rowIndex));
		tfArray.push_back(candidate);
		auto label = layer->findLabel(letters[i] + to_string(rowIndex));
