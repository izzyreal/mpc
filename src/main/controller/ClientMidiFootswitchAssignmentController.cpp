...
void ClientMidiFootswitchAssignmentController::handleEvent(const ClientMidiEvent &e)
{
    std::cout << "[FootswitchAssignment] Handling event type " << e.getMessageType()
        ...
        // Only respond to Control Change messages
    ...
    // MPC behavior: 64+ = pressed, <64 = released
    const bool pressed = value >= 64;

    std::cout << "[FootswitchAssignment] Handling controller " << controller
              << " (val=" << value << (pressed ? ", pressed" : ", released") << ")\n";

    // The screen stores a pair<controller, functionIndex>
    const auto [ctrlNum, functionIndex] = midiSwScreen->getSwitch(controller);

    if (functionIndex < 0 || functionIndex >= static_cast<int>(midiSwScreen->getFunctionNames().size()))
    {
        return;
    }
    ...
    {
        std::cout << "[FootswitchAssignment] Unhandled function: " << fn << std::endl;
        return;
    }
