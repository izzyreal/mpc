    ...
    const ClientHardwareEvent &event)
{
    ...
    const auto programPadIndex =
        isSixteenLevelsEnabled
            ? program->getPadIndexFromNote(assign16LevelsScreen->getNote())
    ...
    auto note = isSixteenLevelsEnabled
    ...
    if (note != NoDrumNoteAssigned)
    {
        registryNoteOnEvent =
            mpc.getPerformanceManager().lock()->registerNoteOn(
                ...
                PhysicalPadIndex(physicalPadIndex), noMidiChannel, screenId,
    ...
    const ClientHardwareEvent &event) const
{
         ...
         isLiveEraseMode, physicalPadIndex, performanceManager]
        {
            ...
            if (!screengroups::isPadDoesNotTriggerNoteEventScreen(p.screenId))
            {
                ...
                if (p.drumNoteNumber != NoDrumNoteAssigned)
                {
                    ...
                    if (!quantizedLockActivated && !isNoteRepeatMode &&
                    !isLiveEraseMode)
                    {
    ...
    performanceManager->enqueueCallback(utils::SimpleAction(
        [performanceManager, physicalPadIndex]
        {
            performanceManager->applyMessageImmediate(
                PhysicalPadReleaseEvent{PhysicalPadIndex(physicalPadIndex)});
        }));
}
