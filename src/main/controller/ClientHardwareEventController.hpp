...
namespace mpc::controller
{
    ...
    public:
        ...
        bool isPhysicallyPressed(const int physicalPadIndex,
        ...
        bool
        isPhysicallyPressedDueToKeyboardInput(const int physicalPadIndex) const
        {
            ...
            const auto pressSource =
                physicalPadPresses.at(physicalPadIndex).inputSource;

            return pressSource == client::event::ClientHardwareEvent::Source::
                                      HostInputKeyboard;
        }

        void registerPhysicalPadPush(
            const int padIndex, const Bank bank,
            ...
            const std::optional<int> drumIndex)
        {
            ...
            physicalPadPresses[padIndex] = {bank, screen, inputSource,
                                            drumIndex};
        }

        client::event::PhysicalPadPress
        registerPhysicalPadRelease(const int padIndex)
        {
    ...
    private:
        ...
        // Maps physical pad index 0 - 15 to info at the time the pad was
        // pushed.
        std::unordered_map<int, client::event::PhysicalPadPress>
            physicalPadPresses;
    };
