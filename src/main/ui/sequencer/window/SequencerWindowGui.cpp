...
int SequencerWindowGui::getChangeBarsLastBar()
{
    return changeBarsLastBar;
}

void SequencerWindowGui::setChangeBarsLastBar(int i, int max)
{
    if(i < 0 || i > max)
        return;

    changeBarsLastBar = i;
    if(changeBarsLastBar < changeBarsFirstBar)
        setChangeBarsFirstBar(changeBarsLastBar, max);

    setChanged();
    notifyObservers(string("changebarslastbar"));
}

int SequencerWindowGui::getChangeBarsFirstBar()
{
    return changeBarsFirstBar;
}

void SequencerWindowGui::setChangeBarsFirstBar(int i, int max)
{
    if(i < 0 || i > max)
        return;

    changeBarsFirstBar = i;
    if(changeBarsFirstBar > changeBarsLastBar)
        setChangeBarsLastBar(changeBarsFirstBar, max);

    setChanged();
    notifyObservers(string("changebarsfirstbar"));
}

int SequencerWindowGui::getChangeBarsNumberOfBars()
{
    return changeBarsNumberOfBars;
}

void SequencerWindowGui::setChangeBarsNumberOfBars(int i, int max)
{
    if(i < 0 || i > (999 - (max + 1)))
        return;
    ...
    setChanged();
    notifyObservers(string("changebarsnumberofbars"));
}

int SequencerWindowGui::getChangeBarsAfterBar()
{
    return changeBarsAfterBar;
}

void SequencerWindowGui::setChangeBarsAfterBar(int i, int max)
{
    if(i < 0 || i > max + 1)
        return;
    ...
    setChanged();
    notifyObservers(string("changebarsafterbar"));
}
