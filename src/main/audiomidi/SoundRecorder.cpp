...
SoundRecorder::SoundRecorder(Mpc &mpc)
    : mpc(mpc),
      leftChannelCopy(INTERNAL_BUF_SIZE),
      rightChannelCopy(INTERNAL_BUF_SIZE),
      unresampledLeft(INTERNAL_BUF_SIZE),
      unresampledRight(INTERNAL_BUF_SIZE),
      resampledLeft(INTERNAL_BUF_SIZE),
      resampledRight(INTERNAL_BUF_SIZE),
      ringLeft(INTERNAL_BUF_SIZE),
      ringRight(INTERNAL_BUF_SIZE)
{
}

unsigned int SoundRecorder::getInputGain() const { return inputGain; }

void SoundRecorder::setInputGain(const unsigned int gain)
{
    if (gain <= 100) inputGain = gain;
}

void SoundRecorder::setArmed(const bool b) { armed = b; }
bool SoundRecorder::isArmed() const { return armed; }
bool SoundRecorder::isRecording() const { return recording.load(); }
                            ...
                            const int engineSampleRateToUse)
{
    ...
    lengthInFramesAtEngineSampleRate = static_cast<int>(std::lround(newLengthInFrames * rateRatio));
    ...
    preRecFramesAtEngineSampleRate = static_cast<size_t>(std::lround(engineSampleRate * 0.001 * preRecMs));

    lengthInFramesAtEngineSampleRate += static_cast<int>(preRecFramesAtEngineSampleRate);

    cancelled = false;
    mode = sampleScreen->getMode();
    if (mode != 2) sound->setMono(true);
...
int SoundRecorder::processAudio(AudioBuffer *buf, const int nFrames)
{
    ...
    {
        if (lastSampleScreenActive) lastSampleScreenActive = false;
    ...
    for (int i = 0; i < nFrames; ++i) {
        const float l = clamp_mean_normalized_float(buf->getChannel(0)[i] * gain);
        const float r = clamp_mean_normalized_float(buf->getChannel(1)[i] * gain);
        leftChannelCopy[i] = l;
    ...
    // When recording, read up to nFrames from ring at readIndex (audio-thread owned)
    int consumed = 0;
    for (int i = 0; i < nFrames; ++i) {
        // Safety: ensure we don't read past writer in a pathological buffer-overrun case.
        // But since audio thread is both writer and reader, and buffer is sized sufficiently,
        // this loop will normally always read `nFrames`.
    ...
    if (engineSampleRate != 44100) {
        if (mode == 0 || mode == 1) {
            ...
            const auto generatedFrameCount = resamplers[0].resample(input, resampledLeft, engineSampleRate, consumed);
            sound->appendFrames(resampledLeft, generatedFrameCount);
        } else {
            const auto genL = resamplers[0].resample(unresampledLeft, resampledLeft, engineSampleRate, consumed);
            const auto genR = resamplers[1].resample(unresampledRight, resampledRight, engineSampleRate, consumed);
            assert(genL == genR);
...
void SoundRecorder::stop()
{
    ...
    if (engineSampleRate != 44100) {
        if (mode == 0 || mode == 1) {
            ...
            const auto remainingL = resamplers[0].wrapUpAndGetRemainder(resampledLeft);
            const auto remainingR = resamplers[1].wrapUpAndGetRemainder(resampledRight);
    ...
    const int lengthInFramesAt44Khz = static_cast<int>(std::lround(lengthInFramesAtEngineSampleRate * invRatio));
    ...
    const auto sampleScreen = mpc.screens->get<ScreenId::SampleScreen>();
    const int preRecFramesAt44Khz = static_cast<int>(44.1 * sampleScreen->preRec);
