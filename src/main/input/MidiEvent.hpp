...
namespace mpc::input {
    ...
    public:
        ...
        void setChannel(const int c) noexcept { channel = static_cast<uint8_t>(c); }
        int getChannel() const noexcept { return static_cast<int>(channel); }

        void setBufferOffset(const double b) noexcept { bufferOffset = b; }
        double getBufferOffset() const noexcept { return bufferOffset; }
