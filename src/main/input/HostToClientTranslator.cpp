    ...
    std::shared_ptr<KeyboardBindings> keyboardBindings)
{
    ...
    switch (hostInputEvent.getSource())
    {
        ...
        case HostInputEvent::Source::KEYBOARD:
        {
            const auto &[keyDown, platformKeyCode, shiftDown, ctrlDown,
                         altDown] = std::get<KeyEvent>(hostInputEvent.payload);
            const auto vmpcKeyCode =
                KeyCodeHelper::getVmpcFromPlatformKeyCode(platformKeyCode);
            const auto binding =
                keyboardBindings->lookupFirstKeyCodeBinding(vmpcKeyCode);
