...
MidiControlCommand::MidiControlCommand(
    ...
    : mpcHardwareLabel(std::move(mpcHardwareLabelToUse)),
      ...
      midiChannelIndex(midiChannelIndexToUse), number(numberToUse), value(-2)
{
    if (midiMessageType != NOTE)
    {
        throw std::invalid_argument(
            "A MidiControlCommand without a value must be of type NOTE");
    }
}
...
MidiControlCommand::MidiControlCommand(
    ...
    : mpcHardwareLabel(std::move(mpcHardwareLabelToUse)),
      midiMessageType(midiMessageTypeToUse),
      midiChannelIndex(midiChannelIndexToUse), number(numberToUse),
      value(valueToUse)
{
    if (midiMessageType != CC)
    {
        throw std::invalid_argument(
            "A MidiControlCommand with a value must be of type CC");
    }
}

MidiControlCommand::MidiControlCommand(const MidiControlCommand &c)
    : mpcHardwareLabel(c.mpcHardwareLabel), midiMessageType(c.midiMessageType),
      midiChannelIndex(c.midiChannelIndex), number(c.number), value(c.value)
{
    if (midiMessageType == NOTE && value != -2)
    {
        throw std::invalid_argument(
            "A MidiControlCommand of type NOTE may not have a value");
    }

    if (midiMessageType == CC && value == -2)
    {
        throw std::invalid_argument(
            "A MidiControlCommand of type CC must have a value");
    }
}
...
{
    return midiMessageType == NONE && midiChannelIndex == -2 && number == -2 &&
           value == -2;
}
...
std::vector<char> MidiControlCommand::toBytes() const
{
    if (midiMessageType == NONE)
    {
        throw std::runtime_error(
            "A MidiControlCommand of type NONE should not be serialized");
    }
...
MidiControlCommand MidiControlCommand::fromBytes(const std::vector<char> &bytes)
{
    ...
    {
        throw std::runtime_error(
            "Erroneous MIDI channel index in persisted MIDI control preset "
            "command");
    }
    ...
    {
        throw std::runtime_error(
            "Erroneous MIDI note or CC number in persisted MIDI control preset "
            "command");
    }
    ...
    return MidiControlCommand(mpcHardwareLabel, CC, midiChannelIndex, number,
                              value);
}
...
MidiControlCommand::MidiMessageType
MidiControlCommand::getMidiMessageType() const
{
...
void MidiControlCommand::setMidiMessageType(
    const MidiMessageType midiMessageTypeToUse)
{
    if (midiMessageTypeToUse == NONE)
    {
        throw std::invalid_argument(
            "After instantiation of a MidiControlCommand, its type may only be "
            "set to type NOTE or CC");
    }
...
void MidiControlCommand::setMidiChannelIndex(const int8_t midiChannelIndexToUse)
{
    ...
    {
        throw std::invalid_argument(
            "After instantiation of a MidiControlCommand, its midiChannelIndex "
            "may only be set to -1 to 15");
    }
...
void MidiControlCommand::setNumber(const int8_t numberToUse)
{
    ...
    {
        throw std::invalid_argument(
            "After instantiation of a MidiControlCommand, its number may only "
            "be set to -1 to 127");
    }
...
void MidiControlCommand::setValue(const int8_t valueToUse)
{
    ...
    {
        throw std::invalid_argument(
            "Only MidiControlCommands of type CC have a value");
    }
