...
namespace mpc::file::wav
{
    ...
    private:
        ...
        const static int SMPL_CHUNK_DATA_SIZE =
            0x04; // 4 Chunk Data Size 36 + (Num Sample Loops * 24) + Sampler
                  // Data
        ...
        const static int MIDI_PITCH_FRACTION =
            0x18; // 4 MIDI Pitch Fraction 0 - 0xFFFFFFFF
        const static int SMPTE_FORMAT =
            0x1C; // 4 SMPTE Format 0, 24, 25, 29, 30
        const static int SMPTE_OFFSET = 0x20; // 4 SMPTE Offset 0 - 0xFFFFFFFF
        const static int NUM_SAMPLE_LOOPS =
            0x1C; // 4 Num Sample Loops 0 - 0xFFFFFFFF
        const static int LIST_OF_SAMPLE_LOOPS_OFFSET =
            0x24; // List of Sample Loops
    ...
    private:
        static unsigned long getLE(std::vector<char> &buffer, unsigned long pos,
                                   int numBytes);
        static void putLE(int val, std::vector<char> &buffer, unsigned long pos,
                          int numBytes);
