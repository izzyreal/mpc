...
namespace mpc::file::all
{
	...
	private:
		static std::vector<mpc::sequencer::Event*> readEvents(std::vector<char> seqBytes);
		static std::vector<std::vector<char>> readEventSegments(std::vector<char> seqBytes);
		double getTempoDouble(std::vector<char> bytePair);

	public:
		static int getNumberOfEventSegmentsForThisSeq(std::vector<char> seqBytes);
