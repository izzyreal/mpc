#include "sequencer/SequenceStateHandler.hpp"

#include "sequencer/Sequence.hpp"
#include "sequencer/SequenceStateView.hpp"
#include "sequencer/Sequencer.hpp"
#include "sequencer/SequencerStateManager.hpp"
#include "sequencer/TrackStateHandler.hpp"
#include "sequencer/SequencerState.hpp"
#include "utils/VariantUtils.hpp"

#include <algorithm>
#include <cstring>
#include <vector>

using namespace mpc::sequencer;
using namespace mpc;

namespace
{
bool tryAcquireTrackLocks(
    SequencerStateManager *const manager,
    const SequenceIndex sequenceIndex,
    const TrackIndex firstTrackIndex,
    const TrackIndex lastTrackIndex,
    std::vector<TrackIndex> &acquired)
{
    for (int rawTrackIndex = firstTrackIndex; rawTrackIndex <= lastTrackIndex;
         ++rawTrackIndex)
    {
        const auto trackIndex = TrackIndex(rawTrackIndex);
        auto &lock = manager->trackLocks[sequenceIndex][trackIndex];
        if (!lock.try_acquire())
        {
            for (auto it = acquired.rbegin(); it != acquired.rend(); ++it)
            {
                manager->trackLocks[sequenceIndex][*it].release();
            }
            acquired.clear();
            return false;
        }

        acquired.push_back(trackIndex);
    }

    return true;
}

bool tryAcquireTrackLocks(
    SequencerStateManager *const manager,
    const SequenceIndex sequenceIndex,
    const std::vector<TrackIndex> &trackIndices,
    std::vector<TrackIndex> &acquired)
{
    for (const auto trackIndex : trackIndices)
    {
        auto &lock = manager->trackLocks[sequenceIndex][trackIndex];
        if (!lock.try_acquire())
        {
            for (auto it = acquired.rbegin(); it != acquired.rend(); ++it)
            {
                manager->trackLocks[sequenceIndex][*it].release();
            }
            acquired.clear();
            return false;
        }

        acquired.push_back(trackIndex);
    }

    return true;
}

void releaseTrackLocks(
    SequencerStateManager *const manager,
    const SequenceIndex sequenceIndex,
    const std::vector<TrackIndex> &acquired)
{
    for (auto it = acquired.rbegin(); it != acquired.rend(); ++it)
    {
        manager->trackLocks[sequenceIndex][*it].release();
    }
}

}

SequenceStateHandler::SequenceStateHandler(SequencerStateManager *manager,
                                           Sequencer *sequencer)
    : manager(manager), sequencer(sequencer)
{
}

SequenceStateHandler::~SequenceStateHandler() {}

void SequenceStateHandler::applyMessage(
    SequencerState &state, std::vector<utils::SimpleAction> &actions,
    const SequenceMessage &msg) const
{
    const auto visitor = Overload{
        [&](const TrackMessage &m)
        {
            manager->trackStateHandler->applyMessage(state, m);
        },
        [&](const SyncTrackEventIndices &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            std::vector<TrackIndex> acquired;
            if (!tryAcquireTrackLocks(
                    manager, m.sequenceIndex, MinTrackIndex,
                    TrackIndex(Mpc2000XlSpecs::TOTAL_TRACK_COUNT - 1),
                    acquired))
            {
                lock.release();
                manager->enqueue(m);
                return;
            }

            const auto positionTicks = Sequencer::quarterNotesToTicks(
                state.transport.positionQuarterNotes);

            auto &tracks = state.sequences[m.sequenceIndex].tracks;

            for (auto &track : tracks)
            {
                if (positionTicks == 0 || !track.eventsHead)
                {
                    track.playEventIndex = EventIndex(0);
                    continue;
                }

                const EventData *ev = track.eventsHead;
                auto eventCounter = EventIndex(0);
                while (ev)
                {
                    if (ev->tick >= positionTicks)
                    {
                        track.playEventIndex = eventCounter;
                        break;
                    }
                    ++eventCounter;
                    ev = ev->next;
                }
            }

            releaseTrackLocks(manager, m.sequenceIndex, acquired);
            lock.release();
        },
        [&](const SetLoopEnabled &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &seq = state.sequences[m.sequenceIndex];
            seq.loopEnabled = m.loopEnabled;

            lock.release();
        },
        [&](const SetSequenceUsed &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &seq = state.sequences[m.sequenceIndex];
            seq.used = m.used;

            lock.release();
        },
        [&](const SetTempoChangeEnabled &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &seq = state.sequences[m.sequenceIndex];
            seq.tempoChangeEnabled = m.tempoChangeEnabled;

            lock.release();
        },
        [&](const SetFirstLoopBarIndex &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &seq = state.sequences[m.sequenceIndex];
            seq.firstLoopBarIndex = m.barIndex;
            if (m.barIndex > seq.lastLoopBarIndex)
            {
                seq.lastLoopBarIndex = m.barIndex;
            }

            lock.release();
        },
        [&](const SetLastLoopBarIndex &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &seq = state.sequences[m.sequenceIndex];
            seq.lastLoopBarIndex = m.barIndex;

            if (m.barIndex != EndOfSequence &&
                m.barIndex < seq.firstLoopBarIndex)
            {
                seq.firstLoopBarIndex = m.barIndex;
            }

            lock.release();
        },
        [&](const SetInitialTempo &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            state.sequences[m.sequenceIndex].initialTempo = m.initialTempo;

            lock.release();
        },
        [&](const SetTimeSignature &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            std::vector<TrackIndex> acquired;
            if (!tryAcquireTrackLocks(
                    manager, m.sequenceIndex, MinTrackIndex,
                    TrackIndex(Mpc2000XlSpecs::TOTAL_TRACK_COUNT - 1),
                    acquired))
            {
                lock.release();
                manager->enqueue(m);
                return;
            }

            const SequenceStateView stateView(state.sequences[m.sequenceIndex]);

            const auto barStart = stateView.getFirstTickOfBar(m.barIndex);

            const auto oldBarLength = stateView.getBarLength(m.barIndex);
            const auto newBarLength = m.timeSignature.getBarLength();
            const auto tickShift =
                static_cast<int>(newBarLength - oldBarLength);

            const auto cutStart = barStart + newBarLength;
            const auto cutEnd = barStart + oldBarLength;

            const auto nextBarStartTick =
                m.barIndex < Mpc2000XlSpecs::MAX_LAST_BAR_INDEX
                    ? stateView.getFirstTickOfBar(m.barIndex + 1)
                    : stateView.getLastTick();

            for (auto &t : state.sequences[m.sequenceIndex].tracks)
            {
                EventData *it = t.eventsHead;
                while (it)
                {
                    EventData *next = it->next;
                    const int tick = it->tick;

                    if (tick >= cutStart && tick < cutEnd)
                    {
                        manager->trackStateHandler->removeEventLocked(t, it);
                    }
                    else if (tick >= nextBarStartTick)
                    {
                        manager->trackStateHandler->updateEventTickLocked(
                            t, it, tick + tickShift);
                    }
                    it = next;
                }
            }

            state.sequences[m.sequenceIndex].timeSignatures[m.barIndex] =
                m.timeSignature;
            state.sequences[m.sequenceIndex].barLengths[m.barIndex] =
                m.timeSignature.getBarLength();

            releaseTrackLocks(manager, m.sequenceIndex, acquired);
            lock.release();
        },
        [&](const SetLastBarIndex &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            state.sequences[m.sequenceIndex].lastBarIndex = m.barIndex;

            lock.release();
        },
        [&](const MoveTrack &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &tracks = state.sequences[m.sequenceIndex].tracks;
            const auto source = m.source;
            const auto dest = m.destination;

            size_t begin = std::min(source, dest);
            size_t end = std::max(source, dest);

            if (source < dest)
            {
                std::rotate(tracks.begin() + source,
                            tracks.begin() + source + 1,
                            tracks.begin() + dest + 1);
            }
            else if (dest < source)
            {
                std::rotate(tracks.begin() + dest, tracks.begin() + source,
                            tracks.begin() + source + 1);
            }

            for (size_t i = begin; i <= end; i++)
            {
                tracks[i].version++;
            }

            lock.release();
        },
        [&](const InsertBars &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            std::vector<TrackIndex> acquired;
            if (!tryAcquireTrackLocks(
                    manager, m.sequenceIndex, MinTrackIndex,
                    TrackIndex(Mpc2000XlSpecs::TOTAL_TRACK_COUNT - 1),
                    acquired))
            {
                lock.release();
                manager->enqueue(m);
                return;
            }

            applyInsertBars(m, state);

            releaseTrackLocks(manager, m.sequenceIndex, acquired);
            lock.release();
        },
        [&](const DeleteBars &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            auto &seq = state.sequences[m.sequenceIndex];
            auto &ts = seq.timeSignatures;
            auto &bl = seq.barLengths;

            const int first = m.firstBarIndex;
            const int last = m.lastBarIndex;
            const int count = last - first + 1;

            const int total = seq.lastBarIndex + 1;

            const int tailBegin = last + 1;
            const int tailEnd = total;

            if (tailBegin < tailEnd)
            {
                std::rotate(ts.begin() + first, ts.begin() + tailBegin,
                            ts.begin() + tailEnd);

                std::rotate(bl.begin() + first, bl.begin() + tailBegin,
                            bl.begin() + tailEnd);
            }

            applyMessage(
                state, actions,
                SetLastBarIndex{m.sequenceIndex, seq.lastBarIndex - count});

            if (seq.lastBarIndex == NoBarIndex)
            {
                applyMessage(state, actions,
                             SetSequenceUsed{m.sequenceIndex, false});
            }

            lock.release();
        },
        [&](const DeleteTrack &m)
        {
            auto &lock = manager->trackLocks[m.sequenceIndex][m.trackIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            applyDeleteTrack(m, state);

            lock.release();
        },
        [&](const DeleteAllTracks &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            std::vector<TrackIndex> acquired;
            if (!tryAcquireTrackLocks(
                    manager, m.sequenceIndex, MinTrackIndex,
                    TrackIndex(Mpc2000XlSpecs::TRACK_COUNT - 1), acquired))
            {
                lock.release();
                manager->enqueue(m);
                return;
            }

            for (int i = 0; i < Mpc2000XlSpecs::TRACK_COUNT; ++i)
            {
                applyDeleteTrack(DeleteTrack{m.sequenceIndex, TrackIndex(i),
                                             m.deviceIndex, m.programChange,
                                             m.busType, m.velocityRatio},
                                 state);
            }

            releaseTrackLocks(manager, m.sequenceIndex, acquired);
            lock.release();
        },
        [&](const UpdateSequenceEvents &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            std::vector<TrackIndex> targetTracks;
            targetTracks.reserve(m.trackSnapshots->size());
            for (const auto &[trackIndex, events] : *m.trackSnapshots)
            {
                (void)events;
                targetTracks.push_back(TrackIndex(trackIndex));
            }
            std::sort(targetTracks.begin(), targetTracks.end());
            targetTracks.erase(
                std::unique(targetTracks.begin(), targetTracks.end()),
                targetTracks.end());

            std::vector<TrackIndex> acquired;
            if (!tryAcquireTrackLocks(
                    manager, m.sequenceIndex, targetTracks, acquired))
            {
                lock.release();
                manager->enqueue(m);
                return;
            }

            applyUpdateSequenceEvents(m, state);

            releaseTrackLocks(manager, m.sequenceIndex, acquired);
            lock.release();
        },
        [&](const UpdateSequenceTracks &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            for (int i = 0; i < Mpc2000XlSpecs::TRACK_COUNT; ++i)
            {
                const auto &source = (*m.trackStates)[i];
                auto &dest = state.sequences[m.sequenceIndex].tracks[i];

                dest.name = source.name;
                dest.busType = source.busType;
                dest.on = source.on;
                dest.velocityRatio = source.velocityRatio;
                dest.programChange = source.programChange;
                dest.deviceIndex = source.deviceIndex;
                dest.used = source.used;
                dest.transmitProgramChangesEnabled =
                    source.transmitProgramChangesEnabled;
                dest.version++;
            }

            lock.release();
        },
        [&](const EraseEvents &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            const auto firstTrackIndex =
                m.trackIndex == AllTracks ? MinTrackIndex : m.trackIndex;
            const auto lastTrackIndex =
                m.trackIndex == AllTracks ? MaxTrackIndex : m.trackIndex;
            std::vector<TrackIndex> acquired;
            if (!tryAcquireTrackLocks(
                    manager, m.sequenceIndex, firstTrackIndex, lastTrackIndex,
                    acquired))
            {
                lock.release();
                manager->enqueue(m);
                return;
            }

            appleEraseEvents(m, state);

            releaseTrackLocks(manager, m.sequenceIndex, acquired);
            lock.release();
        },
        [&](const SetSequenceName &m)
        {
            auto &lock = manager->sequenceLocks[m.sequenceIndex];
            if (!lock.try_acquire())
            {
                manager->enqueue(m);
                return;
            }

            state.sequences[m.sequenceIndex].name.assign(m.name);

            lock.release();
        }};

    std::visit(visitor, msg);
}

void SequenceStateHandler::applyUpdateSequenceEvents(
    const UpdateSequenceEvents &m, SequencerState &state) const
{
    for (auto &[trackIndex, events] : *m.trackSnapshots)
    {
        assert(trackIndex <= Mpc2000XlSpecs::LAST_TRACK_INDEX);
        assert(trackIndex != TempoChangeTrackIndex);

        auto &track = state.sequences[m.sequenceIndex].tracks[trackIndex];

        for (const auto &src : events)
        {
            EventData *e = manager->acquireEvent();
            std::memcpy(e, &src, sizeof(EventData));
            e->sequenceIndex = m.sequenceIndex;
            e->trackIndex = TrackIndex(trackIndex);
            e->prev = nullptr;
            e->next = nullptr;
            manager->insertAcquiredEvent(track, e);
        }
    }
}

void SequenceStateHandler::applyDeleteTrack(const DeleteTrack &m,
                                            SequencerState &state) const
{
    auto &track = state.sequences[m.sequenceIndex].tracks[m.trackIndex];

    manager->trackStateHandler->removeEventsLocked(track);

    track.initializeDefaults();

    track.deviceIndex = m.deviceIndex;
    track.velocityRatio = m.velocityRatio;
    track.busType = m.busType;
    track.programChange = m.programChange;
}

void SequenceStateHandler::appleEraseEvents(const EraseEvents &m,
                                            SequencerState &state) const
{
    static const std::vector eventTypes{
        EventType::NoteOn,          EventType::PitchBend,
        EventType::ControlChange,   EventType::ProgramChange,
        EventType::ChannelPressure, EventType::PolyPressure,
        EventType::SystemExclusive};

    const auto firstTrackIndex =
        m.trackIndex == AllTracks ? MinTrackIndex : m.trackIndex;

    const auto lastTrackIndex =
        m.trackIndex == AllTracks ? MaxTrackIndex : m.trackIndex;

    const auto selectedType = m.type;

    for (auto trackIndex = firstTrackIndex; trackIndex <= lastTrackIndex;
         ++trackIndex)
    {
        auto &track = state.sequences[m.sequenceIndex].tracks[trackIndex];

        EventData *it = track.eventsHead;

        while (it)
        {
            EventData *next = it->next;

            if (m.startTick > it->tick || m.endTick < it->tick)
            {
                it = next;
                continue;
            }

            if (m.erase == 0 ||
                (m.erase == 1 &&
                 it->type != static_cast<EventType>(selectedType)) ||
                m.erase == 2 &&
                    it->type == static_cast<EventType>(selectedType))
            {
                if (it->type == EventType::NoteOn)
                {
                    if (m.noteRange.contains(it->noteNumber))
                    {
                        manager->trackStateHandler->removeEventLocked(track, it);
                    }
                }
                else
                {
                    manager->trackStateHandler->removeEventLocked(track, it);
                }
            }

            it = next;
        }
    }
}
