    ...
    const shared_ptr<MixerControls> &mixerControls, const string &mainStripName)
{
    ...
    for (int i = 0; i < naux; i++)
    {
        ...
        lMixerControls->createStripControls(MixerControlsIds::AUX_STRIP,
                                            busControls->getName(), false);
    }
}
    ...
    const shared_ptr<MixerControls> &mixerControls, int nchannels)
{
    ...
    {
        mixerControls->createStripControls(MixerControlsIds::CHANNEL_STRIP,
                                           to_string(1 + i));
    }
}
    ...
    const shared_ptr<AudioControlsChain> &controls, bool hasMixControls)
{
    ...
    if (stripId == MixerControlsIds::AUX_STRIP)
    {
        ...
        if (busControls)
        {
            lControls->add(make_shared<MixControls>(mixerControls, stripId,
                                                    busControls, true));
        }
    }

    auto mainMixControls = make_shared<MainMixControls>(
        mixerControls, stripId, mixerControls->getMainBusControls(),
        stripId == MixerControlsIds::MAIN_STRIP);

    if (mixerControls->getMainBusControls()->getId() ==
        MixerControlsIds::MAIN_BUS)
    {
