...
int FloatSampleTools::getFormatType(AudioFormat* format)
{
	...
	auto formatType = getFormatType(format->getSampleSizeInBits(), signed_, format->isBigEndian());
	return formatType;
}
...
void FloatSampleTools::byte2float(std::vector<char>& input, int inByteOffset, std::vector<std::valarray<float>>& output, int outOffset, int frameCount, AudioFormat* format)
{
	for (auto channel = 0; channel < format->getChannels(); channel++) {
		if (output.size() < channel) {
			output.emplace_back(std::valarray<float>(frameCount));
		}
		else {
...
void FloatSampleTools::byte2floatGeneric(std::vector<char>& input, int inByteOffset, int inByteStep, std::valarray<float>& output, int outOffset, int sampleCount, AudioFormat* format)
{
...
void FloatSampleTools::byte2floatGeneric(std::vector<char>& input, int inByteOffset, int inByteStep, std::valarray<float>& output, int outOffset, int sampleCount, int formatType)
{
...
{
	if (ditherBits != 0) {
		sample += (((float)rand() / RAND_MAX) + 1) * ditherBits; // apparently this is not super duper distributed random or something. check original.
	}
	if (sample >= 32767.0f) {
		return 32767;
	}
...
void FloatSampleTools::float2byte(std::vector<std::valarray<float>>& input, int inOffset, std::vector<char>& output, int outByteOffset, int frameCount, AudioFormat* format, float ditherBits)
{
	for (auto channel = 0; channel < format->getChannels(); channel++) {
...
void FloatSampleTools::float2byteGeneric(std::valarray<float>& input, int inOffset, std::vector<char>& output, int outByteOffset, int outByteStep, int sampleCount, AudioFormat* format, float ditherBits)
{
...
void FloatSampleTools::float2byteGeneric(std::valarray<float>& input, int inOffset, std::vector<char>& output, int outByteOffset, int outByteStep, int sampleCount, int formatType, float ditherBits)
{
